...
def splitData(gt, args, info):
    ...
    for cls in range(1, len(args.known_classes) + 1):
        ...
        train_index, test_index = train_test_split(locations, train_size=train_num)
    ...
    print('train: %d, close_test: %d, open_test: %d' % (
    len(known_train_index), len(known_test_index), len(unknown_test_index)))

    return {
        'known_train_index': known_train_index,
        'known_test_index': known_test_index, #已知类中的test
        'unknown_test_index': unknown_test_index,  #所有test
        'unknown_unknown_index': unknown_unknown_index  #纯未知类
    }
...
def initData(args, info):
    ...
    data_path = os.path.join('/mnt/HDD/data/zwj/model_1/my_model/datasets', info["path"])
